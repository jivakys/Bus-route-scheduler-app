...
async function showAddScheduleModal() {
  ...
  const [routes, buses] = await Promise.all([fetchRoutes(), fetchBuses()]);
...
async function fetchRoutes() {
  try {
    ...
    const response = await fetch(
      "https://bus-scheduler-backend.onrender.com/api/routes",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function fetchBuses() {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      "https://bus-scheduler-backend.onrender.com/api/buses",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function showEditScheduleModal(schedule) {
            ...
            <form id="editScheduleForm" style="margin-top: 20px;">
                <div class="form-group" style="margin-bottom: 15px;">
                    ...
                    <select id="editRoute" required style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                        <option value="${schedule.route._id}">${
    schedule.route.routeNumber
  } (${schedule.route.startPoint} - ${schedule.route.endPoint})</option>
                    </select>
                </div>
                <div class="form-group" style="margin-bottom: 15px;">
                    ...
                    <select id="editBus" required style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                        <option value="${schedule.bus._id}">${
    schedule.bus.busNumber
  } (${schedule.bus.type})</option>
                    </select>
                </div>
                <div class="form-group" style="margin-bottom: 15px;">
                    ...
                    <input type="datetime-local" id="editDepartureTime" required 
                        value="${new Date(schedule.departureTime)
                          .toISOString()
                          .slice(0, 16)}"
                ...
                <div class="form-group" style="margin-bottom: 15px;">
                    ...
                    <input type="datetime-local" id="editArrivalTime" required 
                        value="${new Date(schedule.arrivalTime)
                          .toISOString()
                          .slice(0, 16)}"
                        style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                </div>
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://bus-scheduler-backend.onrender.com/api/schedules/${schedule._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
